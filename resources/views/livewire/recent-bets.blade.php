    ...
    <x-structure>
        <div class="w-full bg-zinc-200 h-full pb-32">
            ...
            @foreach ($tickets as $ticket)
                <div
                    class="bg-white shadow-lg w-11/12 mx-auto rounded-lg px-3 py-8 mt-10 flex justify-between items-center">
                    <div>
                    ...
                    <div>
                        @if ($ticket->image)
                            <button x-on:click="$dispatch('open-modal', { name: 'modal-{{ $ticket->id }}' })">
                                Preview
                            </button>
                        @endif
                    </div>
        ...
        @foreach ($tickets as $ticket)
            <div x-data="{ show: false }" x-show="show" class="fixed inset-0 flex items-center justify-center z-50"
                ...
                style="display: none">
                <!-- Overlay -->
                <div x-on:click="show = false" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm">
                </div>
                ...
                <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg z-10">
                    ...
                    <div class="flex justify-between items-center border-b pb-3">
                        <button x-on:click="$dispatch('close-modal')" class="text-gray-600 hover:text-gray-900">
                            ✖
                        </button>
                    </div>
